...
<?php if (isset($_GET['submitted'])): ?>
  <div class="alert alert-success animate-in" style="position:fixed;top:20px;right:20px;z-index:9999;max-width:400px;box-shadow:var(--shadow-lg); border:none; background:var(--trouve); color:white; border-radius:var(--radius); padding:1.2rem; display:flex; align-items:center; gap:1rem;">
    <i class="fas fa-check-circle" style="font-size:1.5rem;"></i> 
    <div>
      <strong style="display:block;">Succès !</strong>
      <span style="font-size:0.9rem; opacity:0.9;">Annonce soumise avec succès. Elle sera publiée après validation.</span>
    </div>
  </div>
<?php endif; ?>
...
<nav class="navbar">
  <div class="nav-inner">
    ...
    <div class="nav-links">
      <a href="<?= url('index.php') ?>" class="<?= !$type ? 'active' : '' ?>"><i class="fas fa-th-large"></i> Explorer</a>
      <a href="?type=perdu" class="<?= $type==='perdu' ? 'active' : '' ?>"><i class="fas fa-question-circle"></i> Perdus</a>
      <a href="?type=trouve" class="<?= $type==='trouve' ? 'active' : '' ?>"><i class="fas fa-hand-holding-heart"></i> Trouvés</a>
    </div>
    <div class="nav-actions">
      <a href="<?= url('declarer.php') ?>" class="btn-declare"><i class="fas fa-plus-circle"></i> Publier</a>
      <?php if ($user): ?>
        <a href="<?= in_array($user['role'],['admin','personnel']) ? url('admin/index.php') : url('espace-parent.php') ?>" class="btn-admin">
          <i class="fas fa-user-circle"></i> <?= clean($user['prenom']) ?>
          <?php if (in_array($user['role'],['admin','personnel'])): ?><span class="admin-tag">Admin</span><?php endif; ?>
        </a>
        <a href="<?= url('logout.php') ?>" class="btn-logout" title="Déconnexion"><i class="fas fa-sign-out-alt"></i></a>
      <?php else: ?>
...
<div class="type-tabs container">
  <a href="<?= $type ? '?'.http_build_query(array_merge($_GET,['type'=>''])) : url('index.php') ?>" class="tab <?= !$type?'active':'' ?>">
    <i class="fas fa-th"></i> Tous <span class="badge"><?= $total ?></span>
  </a>
  <a href="?<?= http_build_query(array_merge($_GET,['type'=>'perdu','page'=>1])) ?>" class="tab <?= $type==='perdu'?'active red':'' ?>">
    <i class="fas fa-question-circle"></i> Perdus
  </a>
  <a href="?<?= http_build_query(array_merge($_GET,['type'=>'trouve','page'=>1])) ?>" class="tab <?= $type==='trouve'?'active green':'' ?>">
    ...
    <article class="card <?= $a['type'] ?>">
      ...
      <div class="card-body">
        ...
        <div class="card-meta">
          ...
          <span><i class="fas fa-calendar-alt"></i> <?= $a['date_objet'] ? date('d/m/Y', strtotime($a['date_objet'])) : date('d/m/Y', strtotime($a['date_creation'])) ?></span>
        </div>
      </div>
      <div class="card-footer">
        <a href="<?= url('detail.php') ?>?id=<?= $a['id'] ?>" class="btn-detail">Voir détail <i class="fas fa-arrow-right"></i></a>
      </div>
    </article>
    <?php endforeach; ?>
  <?php endif; ?>
...
<footer class="site-footer">
  <div class="footer-inner">
    ...
    <a href="<?= url('declarer.php') ?>" class="btn-declare-footer"><i class="fas fa-plus-circle"></i> Déclarer un objet</a>
  </div>
</footer>
